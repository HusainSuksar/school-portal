...
import { Users, Calendar, Award, BookOpen, Clock, ChevronDown, Activity, CheckCircle2, AlertCircle, Shield } from 'lucide-react';
...
export default function ParentDashboard() {
  ...
  const fetchFamilyData = async () => {
    ...
    // Fetch all students linked to this parent, including their class details
    ...
    if (linkedStudents && linkedStudents.length > 0) {
      ...
      setActiveChildId(linkedStudents[0].id); // Default to the first child
    }
  ...
  const fetchChildMetrics = async (childId) => {
    ...
    if (attendanceData) {
      ...
      setAttendanceStats({
        percentage,
        recent: attendanceData.slice(0, 5) // Grab the 5 most recent days
      });
    }
    ...
    const { data: gradesData } = await supabase
      .from('grades')
      .select('term, numeric_mark, letter_grade, subjects(name)')
      .eq('student_id', childId)
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-6 pb-12">
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12 text-center flex flex-col items-center">
          ...
          <p className="text-slate-500 mt-2 max-w-md">Your parent account is active, but no students are currently linked to it. Please contact the school administration to map your children to your ITS number.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-12">
      ...
      {/* Dynamic Header & Sibling Selector */}
        ...
        {children.length > 1 && (
          <div className="relative group">
            <select
              ...
              className="appearance-none bg-school-navy text-white font-bold py-2.5 pl-4 pr-10 rounded-lg shadow-md cursor-pointer hover:bg-slate-800 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
            >
              {children.map(child => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Left Column: Digital ID & Attendance Summary */}
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative">
            ...
            <div className="px-6 pb-6 pt-0 relative">
              <div className="w-20 h-20 bg-white rounded-full p-1.5 absolute -top-10 shadow-sm border border-slate-100">
              ...
              <div className="pt-12">
                ...
                <div className="mt-6 pt-4 border-t border-slate-100 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Father</p>
                      <p className="text-sm font-bold text-slate-700 line-clamp-1">{activeChild?.father_name || '--'}</p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Mother</p>
                      <p className="text-sm font-bold text-slate-700 line-clamp-1">{activeChild?.mother_name || '--'}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div className="flex items-end gap-2 mb-2">
              <span className={`text-4xl font-black ${
                attendanceStats.percentage >= 90 ? 'text-emerald-500' : 
                attendanceStats.percentage >= 75 ? 'text-amber-500' : 'text-red-500'
              }`}>
            ...
            <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden mb-6">
              <div 
                className={`h-full rounded-full transition-all duration-1000 ${
                  attendanceStats.percentage >= 90 ? 'bg-emerald-500' : 
                  attendanceStats.percentage >= 75 ? 'bg-amber-500' : 'bg-red-500'
                }`}
        ...
        {/* Right Column: Academic Report Card */}
            ...
            <div className="p-6 flex-1 overflow-y-auto custom-scrollbar">
              {grades.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center py-12">
                  ...
                  <p className="text-sm text-slate-400 mt-1 max-w-sm">Teachers have not yet published term grades for this student. Check back at the end of the term.</p>
                </div>
              ) : (
                <div className="space-y-8">
                  {/* Group grades by Term automatically */}
                    ...
                    return (
                      <div key={term}>
                        <h4 className="text-sm font-bold text-indigo-600 uppercase tracking-wider mb-3 flex items-center gap-2">
                          <Calendar className="w-4 h-4" /> {term}
                        </h4>
                              ...
                              {termGrades.map((grade, idx) => (
                                <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                  ...
                                  <td className="p-3 text-sm font-bold text-slate-700 text-right">
                                    {grade.letter_grade ? (
                                      <span className="inline-flex items-center justify-center w-8 h-8 rounded bg-indigo-50 text-indigo-700 border border-indigo-100">
                                        {grade.letter_grade}
                                      </span>
                                    ) : (
                                      <span className="text-lg">{grade.numeric_mark}%</span>
                                    )}
                                  </td>
                                </tr>
                              ))}
