...
import { Book, Search, Filter, TrendingUp, AlertTriangle, Clock, User, Award } from 'lucide-react';
...
export default function SchoolLogBook() {
  ...
  const [filterType, setFilterType] = useState('All'); // 'All', 'Tashjee', 'Tanbeeh'

  const fetchLogs = async () => {
    setIsLoading(true);
    
    // Fetch the last 100 behavior logs across the entire school
    // Joining students, classes, and profiles (the teacher who logged it)
    const { data, error } = await supabase
      ...
      .select(`
        ...
        reason,
        logged_at,
        students (full_name, its_number, classes(class_name)),
        profiles (full_name)
      `)
  ...
  // Apply filters and search locally for instant UI response
        ...
        <div className="flex bg-slate-100 p-1 rounded-lg border border-slate-200 w-full md:w-auto">
          <button 
            ...
            className={`flex-1 md:flex-none px-4 py-1.5 rounded-md text-sm font-bold transition-all ${
              filterType === 'All' ? 'bg-white text-school-navy shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            All Logs
          </button>
          <button 
            onClick={() => setFilterType('Tashjee')}
            className={`flex-1 md:flex-none px-4 py-1.5 rounded-md text-sm font-bold transition-all flex items-center justify-center gap-1 ${
              filterType === 'Tashjee' ? 'bg-emerald-100 text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-emerald-600'
            }`}
          >
            <TrendingUp className="w-4 h-4" /> Tashjee
          </button>
          <button 
            onClick={() => setFilterType('Tanbeeh')}
            className={`flex-1 md:flex-none px-4 py-1.5 rounded-md text-sm font-bold transition-all flex items-center justify-center gap-1 ${
              filterType === 'Tanbeeh' ? 'bg-red-100 text-red-700 shadow-sm' : 'text-slate-500 hover:text-red-600'
            }`}
          >
            <AlertTriangle className="w-4 h-4" /> Tanbeeh
          </button>
        </div>
      </div>
                    ...
                    <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="p-4">
                        <span className={`flex items-center justify-center w-8 h-8 rounded-full ${
                          log.log_type === 'Tashjee' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'
                        }`}>
                      ...
                      {/* Student Info */}
                      <td className="p-4">
                        <p className="text-sm font-bold text-school-navy">{log.students?.full_name}</p>
                        <p className="text-xs text-slate-500 font-mono">{log.students?.its_number}</p>
                      </td>
                      
                      {/* Class */}
                      <td className="p-4">
                        <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded text-xs font-bold">
                          {log.students?.classes?.class_name || 'N/A'}
                        </span>
                      </td>
                      ...
                      <td className="p-4 max-w-xs">
                        <p className="text-sm font-bold text-slate-700">
                          <span className={log.log_type === 'Tashjee' ? 'text-emerald-600' : 'text-red-600'}>
                            {log.log_type === 'Tashjee' ? '+' : '-'}{log.points} Pts
                          </span>
                        </p>
                        <p className="text-xs text-slate-500 truncate mt-0.5" title={log.reason}>{log.reason}</p>
                      </td>
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-slate-600">{log.profiles?.full_name}</span>
                        </div>
                      </td>
                      ...
                      <td className="p-4 text-right">
                        ...
                        <p className="text-xs text-slate-400 flex items-center justify-end gap-1 mt-0.5">
                          <Clock className="w-3 h-3" /> {new Date(log.logged_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </p>
                      </td>
                    </tr>
                  ))
